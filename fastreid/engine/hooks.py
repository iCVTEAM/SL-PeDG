...
class EvalHook(HookBase):
    ...
    # def after_step(self):
    #     if (self.trainer.iter + 1) % 300 == 0 or self.trainer.iter == 0:
    #         self._do_eval()
...
class LayerFreeze(HookBase):
    def __init__(self, model, freeze_layers, freeze_iters, fc_freeze_iters, open_target_layers):
        self._logger = logging.getLogger(__name__)
        ...
        self.open_target_layers = open_target_layers
    ...
    def open_specific_layer(self):
        for name, module in self.model.named_modules():
            for target_layer in self.open_target_layers:
                if len(target_layer) != 0 and target_layer in name:
                    module.train()
                    for p in module.parameters():
...
class DropoutSGDHook(HookBase):
    def __init__(self, optimizer, update_epochs, end_layer, step, window_size, open_iters,
                 dropout_all_epoch, end_all_epochs, prob_list, prob_steps):
        self.optimizer = optimizer
        self._update_epochs = update_epochs
        self._end_layers = end_layer
        self._step = step
        self._w_size = window_size
        self._open_iters = open_iters
        self._dropout_all_epochs = dropout_all_epoch
        self._end_all_epochs = end_all_epochs
        self._prob_idx = 0
        self._prob_list = prob_list
        self._prob_steps = prob_steps

    def before_step(self):
        if self.trainer.iter >= self._open_iters:
            self.optimizer.open_dropout = True

        self.trainer.storage.put_scalar('drop_prob', self.optimizer.p_prob, smoothing_hint=False)
    

    def after_epoch(self):
        next_epoch = self.trainer.epoch + 1

        if self._prob_idx < len(self._prob_list):
            if next_epoch >= self._prob_steps[self._prob_idx]:
                self.optimizer.p_prob = self._prob_list[self._prob_idx]
                self._prob_idx += 1

        if self._dropout_all_epochs <= next_epoch < self._end_all_epochs:
            self.optimizer.cur_layers = 0
            self.optimizer.max_layers = self._end_layers

        elif next_epoch % self._update_epochs == 0 and self.optimizer.open_dropout:
            self.optimizer.cur_layers = (self.optimizer.cur_layers + self._step) % (
                    self._end_layers + 1 - self._w_size)

            self.optimizer.max_layers = min(self.optimizer.cur_layers + self._w_size, self._end_layers)
