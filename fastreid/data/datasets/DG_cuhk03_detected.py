...
class DG_CUHK03_detected(ImageDataset):
    ...
    def __init__(self, root='datasets', split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, **kwargs):
        ...
        for img_path, pid, camid in train:
            ...
            if new_pid not in self.train_pid_list:
                self.train_pid_list.append(new_pid)

            tmp_train.append((img_path, new_pid, new_camid, self.domain_id))

        for img_path, pid, camid in split['query']:
            new_pid = self.dataset_name + "_" + str(pid)
            new_camid = self.dataset_name + "_" + str(camid)
            if new_pid not in self.train_pid_list:
                self.train_pid_list.append(new_pid)
        ...
        for img_path, pid, camid in split['gallery']:
            new_pid = self.dataset_name + "_" + str(pid)
            new_camid = self.dataset_name + "_" + str(camid)
            if new_pid not in self.train_pid_list:
                self.train_pid_list.append(new_pid)
        ...
        super(DG_CUHK03_detected, self).__init__(train, query, gallery, **kwargs)
