...
from fastreid.utils.features import FeatureListNet, FeatureDictNet, FeatureHookNet
from fastreid.utils.hub import has_hf_hub, download_cached_file, load_state_dict_from_hf, load_state_dict_from_url
from fastreid.layers.vit_layers import Conv2dSame, Linear
...
def load_pretrained(model, default_cfg=None, num_classes=1000, in_chans=3, filter_fn=None, strict=True, progress=False):
    ...
    if hf_hub_id and has_hf_hub(necessary=not pretrained_url):
        ...
        state_dict = load_state_dict_from_url(pretrained_url, model_dir=default_cfg['pretrain_dir'],
    ...
    model.load_state_dict(state_dict, strict=strict)
    _logger.info(f'Finish loading state dictionary!')
