...
def domain_center_on_dataset(model, data_loader):
    num_devices = comm.get_world_size()
    logger = logging.getLogger(__name__)
    logger.info("Start to compute domain center on {} images".format(len(data_loader.dataset)))
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            domain_center = mean_feat.cpu() / total + domain_center
            if torch.cuda.is_available():
                torch.cuda.synchronize()
            total_compute_time += time.perf_counter() - start_compute_time
    ...
    # NOTE this format is parsed by grep
    logger.info(
        "Total computation time: {} ({:.6f} s / batch per device, on {} devices)".format(
            total_time_str, total_time / (total - num_warmup), num_devices
        )
    )
    total_compute_time_str = str(datetime.timedelta(seconds=int(total_compute_time)))
    logger.info(
        "Total computation pure compute time: {} ({:.6f} s / batch per device, on {} devices)".format(
            total_compute_time_str, total_compute_time / (total - num_warmup), num_devices
        )
    )
