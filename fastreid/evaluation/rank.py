...
def cgm(matches, g_pids_order, q_pid, g_camids_order, q_camid):
    ...
    # match_q_pids = g_pids_order == q_pid
    ...
    for cam in cam_list:
        ...
        num_rel = cam_cmc.sum()
        # tmp_cmc = cam_cmc.cumsum()
        invert_tmp_cmc = invert_cam_cmc.cumsum()
        # tmp_dc = [x / discount[invert_tmp_cmc[i]] for i, x in enumerate(tmp_cmc)]
        tmp_dc = [1 / (x + 1) for i, x in enumerate(invert_tmp_cmc)]

        tmp_dc = np.asarray(tmp_dc) * cam_cmc
        dc = tmp_dc.sum() / num_rel

        cam_measure_list.append(dc)

    return np.mean(cam_measure_list)
...
def eval_cuhk03(distmat, q_pids, g_pids, q_camids, g_camids, max_rank):
    ...
    return all_cmc, all_AP, all_INP, all_CMG
...
def eval_market1501(distmat, q_pids, g_pids, q_camids, g_camids, max_rank):
    ...
    for q_idx in tqdm.tqdm(range(num_q)):
        ...
        # compute CamAP
        # cam_cgm = cgm(matches[q_idx], g_pids[order], q_pid, g_camids[order], q_camid)
        # all_CMG.append(cam_cgm)
        # all_CMG.append(0.0)
